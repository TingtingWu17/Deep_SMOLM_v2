...
def _eval_val_metric_zoom(self, data, label, output): # Added for only localization
    rawx = data[:,0,:,:]
    rawy = data[:,1,:,:]

    estimated_loc = output[:,:,:,:]
    gt_loc = label[:,0,:,:] 
    ...
    fig = plot_comparison_v2(rawx[samp_to_show], 
                            rawy[samp_to_show], 
...
def plot_angle_scatters(angle_pred_list, angle_gt_list):
    ...
    plt.scatter(theta_gt, theta_pred, alpha=0.2)
    ...
    plt.scatter(phi_gt, phi_pred, alpha=0.2)
    ...
    if np.size(angle_gt_list)>0:
        ...
        indx = differ<45
        degree_mse2 = np.mean((differ[indx]) ** 2)
    ...
    plt.scatter(gamma_gt2, gamma_pred,alpha=0.2)
...
def plot_I_scatters(I_pred, I_gt):
    #angle_pred_list = np.array(angle_pred_list)
    #angle_gt_list = np.array(angle_gt_list)

    I_gt2 = 0+np.random.rand(np.shape(I_gt)[0])
    fig = plt.figure(figsize = (5,5))
    plt.scatter(I_gt, I_pred-I_gt, alpha=0.1)
    plt.plot(I_gt, I_gt-I_gt, color = "orange")
    plt.xlabel("GT theta value")
    plt.ylabel("Predicted theta value")
    if np.size(I_gt)>0:
        I_mse = np.mean((I_gt-I_pred) ** 2)
        I_rmse = np.round_(I_mse**(1/2), 2)
        I_bias = np.round_(np.mean(I_pred-I_gt),2)
    else:
        I_rmse=1e10
        I_bias = 0
    plt.title(f"The intensity RMSE is {I_rmse}+the intensity bais is {I_bias}")
...
def writeResults2comet(self,figure_names=None,figures=None):
    for ii in range(np.size(figure_names)):
        self.writer.add_plot(figure_names[ii], figures[ii])
