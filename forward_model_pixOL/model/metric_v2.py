...
def postprocessingv2(config, output, idx):
    ...
    for ii in range(B):
        pre_est_cur = output[ii,:,:,:]
        #I_thresh = 50
        x_est_save,y_est_save, est_img_crop = postprocessing_loc_v3(pre_est_cur, I_thresh)
        N_SM = np.size(x_est_save)
        if N_SM==0:
           aaa=1
        else:
    ...
    if has_SM==0:
        ...
        #calculate metric
        bias_con_x_all, bias_con_y_all,I_est_all, orien_est,M_est = loc_angle_est2(config, est_img_crop_all, x_GT,y_GT, 
                                                                x_est_save_all,y_est_save_all)       
...
def postprocessing_loc_v3(est_images, I_thresh):
    # pre_est: output image from the network
    channels = np.size(est_images,axis=0)
    rad = 3
    I_img = est_images[6,:,:]
