...
save_folder = 'C:\Users\wu.t\OneDrive - Washington University in St. Louis\github\data for Deep-SMOLM\training_20210331_SNR1000vs2_large_variance\'; 
% ********************************
...
[sXX,sYY,sZZ,sXY,sXZ,sYZ,signal_SMs] = generate_non_physical_uniform_sM(signal,n_SMs);
%[muxx,muyy,muzz,muxy,muxz,muyz] = Quickly_rotating_matrix_angleD_gamma_to_M(thetaD_SMs,phiD_SMs,gamma_SMs);
sM = [sXX.';sYY.';sZZ.';sXY.';sXZ.';sYZ.'];
I_SMs = basis_matrix_opt*sM;
...
for i = 1:n_SMs
Ix = Ix+imtranslate(I_SMsx(:,:,i),[x_SMs(i),y_SMs(i)],'bicubic');
Iy = Iy+imtranslate(I_SMsy(:,:,i),[x_SMs(i),y_SMs(i)],'bicubic');
...
I_sXX = I_sXX+temp*sXX(i);
I_sYY = I_sYY+temp*sYY(i);
I_sZZ = I_sZZ+temp*sZZ(i);
I_sXY = I_sXY+temp*sXY(i);
I_sXZ = I_sXZ+temp*sXZ(i);
I_sYZ = I_sYZ+temp*sYZ(i);
end
...
image_GT_up(2,:,:) = zeros(size(I_theta_up));
image_GT_up(3,:,:) = zeros(size(I_phi_up));
image_GT_up(4,:,:) = zeros(size(I_gamma_up));
...
GT_list(2,:)=y_phy;
GT_list(3,:)=I_grd;
GT_list(4,:)=zeros(size(thetaD_grd));
GT_list(5,:)=zeros(size(phiD_grd));
GT_list(6,:)=zeros(size(gamma_grd));
