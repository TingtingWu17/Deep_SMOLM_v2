...
% ********************************
save_folder = '/home/wut/Documents/Deep-SMOLM/data/opt_PSF_data/validation_SNR380vs5_min7_U8_unifAngleSample_2021Jan7/'; 
% ********************************
...
angle_states = 20;
gamma_states = 4;
x1 = linspace(-1,1,angle_states);
x2 = linspace(-1,1,angle_states);

[X,Y]=meshgrid(x1,x2);
mux_list = 2*X.*sqrt(1-X.^2-Y.^2);
muy_list = 2*Y.*sqrt(1-X.^2-Y.^2);
muz_list = 1-2*(X.^2+Y.^2);

indx = muz_list<0 | X.^2+Y.^2>1;
mux_list(indx)=[];
muy_list(indx)=[];
muz_list(indx)=[];
mux_list = reshape(mux_list,1,[]);
muy_list = reshape(muy_list,1,[]);
muz_list = reshape(muz_list,1,[]);

omega = linspace(0,2*pi,5);
gamma_list = 1-3*omega/4/pi+omega.^2/8/pi^2;
...
for ii = 1:length(mux_list)
        for kk = 1:length(gamma_list)
            for ll = 1:num_each_state
                zz=zz+1
                mux_cur = mux_list(ii);
                muy_cur = muy_list(ii);
                muz_cur = muz_list(ii);
                gamma_cur = gamma_list(kk);
                thetaD_cur = asin(muz_cur)/pi/180;
                phiD_cur = atan(muy_cur./mux_cur)/pi/180;
                ...
                thetaD_SMs = thetaD_cur*ones(1,n_SMs); %theta angle of SMs, note theta is in the range of (0,90) degree
                phiD_SMs = phiD_cur*ones(1,n_SMs); %phi angle of SMs, note phi is in the range of (0,180) degree
                gamma_SMs = gamma_cur*ones(1,n_SMs); %gamma (orientaiton constraint) is used to represent alpha angle. it is in the range of (0,1)
