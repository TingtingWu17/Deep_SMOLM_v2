...
save_folder = '/home/wut/Documents/Deep-SMOLM/data/opt_PSF_data_1000vs2/training_20220718_retrieve_pixOL_com_SNR2000vs6_randomxybkgration_corrected_gamma_linear_photon_poisson_distribution_little_3D_0214pmask/'; 
% ********************************
...
normalize_coeff = max(max(h));
h = h./normalize_coeff;

% dh_x = x./(h_sigma^2).*exp(-(x.^2+y.^2)/(2*h_sigma^2))./normalize_coeff;
% dh_y = y./(h_sigma^2).*exp(-(x.^2+y.^2)/(2*h_sigma^2))./normalize_coeff;
...
h_basis = I_basis; %h_dx_basis = I_basis;  h_dy_basis = I_basis;
h_basis(round((size(I_basis,1)+1)/2)+[-(h_shape(1)-1)/2:(h_shape(1)-1)/2],round((size(I_basis,2)+1)/2)+[-(h_shape(1)-1)/2:(h_shape(1)-1)/2]) = h;
%h_dx_basis(round((size(I_basis,1)+1)/2)+[-(h_shape(1)-1)/2:(h_shape(1)-1)/2],round((size(I_basis,2)+1)/2)+[-(h_shape(1)-1)/2:(h_shape(1)-1)/2]) = dh_x;
%h_dy_basis(round((size(I_basis,1)+1)/2)+[-(h_shape(1)-1)/2:(h_shape(1)-1)/2],round((size(I_basis,2)+1)/2)+[-(h_shape(1)-1)/2:(h_shape(1)-1)/2]) = dh_y;
...
%temp = imtranslate(h_basis,round([(x_SMs(i)*upsampling_ratio),(y_SMs(i)*upsampling_ratio)]));
%temp_dhx = imtranslate(h_dx_basis,round([(x_SMs(i)*upsampling_ratio),(y_SMs(i)*upsampling_ratio)]));
%temp_dhy = imtranslate(h_dy_basis,round([(x_SMs(i)*upsampling_ratio),(y_SMs(i)*upsampling_ratio)]));
%remain = [(x_SMs(i)*upsampling_ratio),(y_SMs(i)*upsampling_ratio)] - round([(x_SMs(i)*upsampling_ratio),(y_SMs(i)*upsampling_ratio)]);
%temp = temp+temp_dhx*remain(1)+temp_dhy*remain(2);
