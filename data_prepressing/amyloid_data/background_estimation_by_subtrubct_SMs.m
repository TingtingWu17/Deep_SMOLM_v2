...
SMLM_offsetName = [fileFolder,'processes data\20220214_offSet_for_amyloid.mat'];
Nimg = 2000;
...
%%
fileName1 = [[fileFolder,'processes data\BKG_list_data145_149\data'],  num2str(ID(kk)) '_xch.csv'];
...
fileName1 = [[fileFolder,'processes data\BKG_list_data145_149\data'],  num2str(ID(kk)) '_ych.csv'];
data = readtable(fileName1);
...
for i=1:50:Nimg
    ...
    SMLM_bkgX(:,:,count) = imgaussfilt(back_cur(:,:,:),30);
...
for i=1:50:Nimg
    ...
    back_cur(back_cur<0)=0;
    back_cur(isnan(back_cur))=nanmean(back_cur(round(H/2)+range,round(W/2)+range),'all');
    SMLM_bkgY(:,:,count) = imgaussfilt(back_cur(:,:,:),30);
...
load(strcat(fileFolder,'processes data\saved_beads_loc_for_tform\tformx2y_y_center_469_198_FoV_350_for data103_149.mat'));
Nimg = 2000;
FoV = [320,320]; 
N_FoV = [4,4];
FoV_each = 96;
...
SMLM_save_Nmae = ['processes data\data',num2str(ID(kk)),'_bkg_centerY_y',num2str(ROI_centerY(1)),'_x_',num2str(ROI_centerY(2)),'_','FoV',num2str(FoV(1)),'_',num2str(FoV(2)),'_',num2str(count),'th_FoV','.mat'];
